...
public class KklBookingServerImpl extends RoomBookingInterfacePOA 
{
	...
	{
		String check="false";
		defaultLogFile="E:\\KKLCampus_Server_Logs_"+username+"_log.txt";
	...
	public String getAvailableTimeSlot(String date,String username,String campus)
	{
		...
		System.out.println(" WST : "+modifiedSentencewst);
		
		System.out.println(" DVL : "+modifiedSentencedvl);
		
		System.out.println(" KKL : "+modifiedSentencekkl);
		
		System.out.println("Return value before sending to client : "+return_value);
		return return_value;
} 
	...
	try
	{
		...
		{	
			...
			while (itold.hasNext()) 
			{
				...
				KklRoomRecord rasold=(KklRoomRecord)pairold.getValue();
				...
				if(rasold.date.equals(olddate)==true && rasold.timeslot.equals(oldtimeslot)==true && rasold.roomno.equals(oldroomno)==true && rasold.bookedby==username )
				{
					...
					while (itnew.hasNext()) 
					{
						...
						KklRoomRecord rasnew=(KklRoomRecord)pairnew.getValue();
						if(rasnew.date.equals(newdate)==true && rasnew.timeslot.equals(newtimeslot)==true && rasnew.roomno.equals(newroomno)==true && rasnew.bookedby=="-" )
						{
								...
								KklRoomRecord rc=(KklRoomRecord)pairnew.getValue();
								synchronized(this)
								{
									roomrec.remove(pairnew.getKey(),(KklRoomRecord)pairnew.getValue());
									rc.bookedby=username;
									rc.bookingid=username+""+(int)(new Date().getTime()/1000);
									bookingid=rc.bookingid;
									System.out.println("\n\n Room is booked by : "+rc.bookedby);
								...
								KklRoomRecord rcold=(KklRoomRecord)pairold.getValue();
								synchronized(this)
								{
									roomrec.remove(pairold.getKey(),(KklRoomRecord)pairold.getValue());
									rcold.bookedby="-";
									rcold.bookingid="-";
									System.out.println("\n\n Room is booked by : "+rc.bookedby);
									roomrec.put(k,(KklRoomRecord)rcold);
				...
				else if((campus.equals("DVLSERVER")|| campus.equals("KKLSERVER") || campus.equals("WSTSERVER") && !kklbooked.equals("true")))
				{
					...
					if(return_valuewst.equals("Successful") || return_valuedvl.equals("Successful"))
					{
						...
						synchronized(this)
						{
							...
							roomrec.put(n,(KklRoomRecord)rcold);
							log="Change Successful\t"+" Room No : "+ newroomno+"\t Booking Date : "+newdate+"\t Timeslot : "+newtimeslot+"\t Booked by : "+username;//+"\t Booking ID : "+bookingid;
