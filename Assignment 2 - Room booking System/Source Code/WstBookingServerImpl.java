...
public class WstBookingServerImpl extends RoomBookingInterfacePOA 
{
	private ORB orb;
	...
	public String getAvailableTimeSlot(String date,String username,String campus)
	{
		...
		System.out.println("Date in get available time slot : "+date);
		System.out.println("username in get available time slot : "+username);
		
		System.out.println("campus in get available time slot : "+campus);
		...
		if(campus.equals("DVLSERVER")|| campus.equals("KKLSERVER")|| campus.equals("WSTSERVER"))
		{
			...
			System.out.println("WST SERVER called this function now : \n");
			try
			{
				if(!roomrec.isEmpty())
				{	
		...
		else
		{
			try
			{
				...
				System.out.println("request to be sent to dvl server  :"+sentencedvl);
				...
				System.out.println("count in dvl : "+modifiedSentencedvl.trim());
				System.out.println("request to be sent to dvl server  ");
				...
				System.out.println("count in kkl : "+modifiedSentencekkl.trim());
				...
				System.out.println("count in dvl : "+modifiedSentencewst.trim());
			}catch(Exception e){}		
		}
			/*
		try (FileWriter f = new FileWriter(defaultLogFile, true); BufferedWriter b = new BufferedWriter(f); PrintWriter p = new PrintWriter(b);)
	...
	try
	{
		...
		{	
			...
			while (itold.hasNext()) 
			{
				...
				WstRoomRecord rasold=(WstRoomRecord)pairold.getValue();
				//System.out.println(rasold.date.equals(date)+","+rasold.timeslot.equals(timeslot)+","+rasold.roomno.equals(no));
				...
				if(rasold.date.equals(olddate)==true && rasold.timeslot.equals(oldtimeslot)==true && rasold.roomno.equals(oldroomno)==true && rasold.bookedby==username )
				{
					...
					while (itnew.hasNext()) 
					{
						...
						WstRoomRecord rasnew=(WstRoomRecord)pairnew.getValue();
						if(rasnew.date.equals(newdate)==true && rasnew.timeslot.equals(newtimeslot)==true && rasnew.roomno.equals(newroomno)==true && rasnew.bookedby=="-" )
						{
								...
								WstRoomRecord rc=(WstRoomRecord)pairnew.getValue();
								synchronized(this)
								{
									roomrec.remove(pairnew.getKey(),(WstRoomRecord)pairnew.getValue());
									rc.bookedby=username;
									rc.bookingid=username+""+(int)(new Date().getTime()/1000);
									bookingid=rc.bookingid;
									System.out.println("\n\n Room is booked by : "+rc.bookedby);
								...
								WstRoomRecord rcold=(WstRoomRecord)pairold.getValue();
								synchronized(this)
								{
									roomrec.remove(pairold.getKey(),(WstRoomRecord)pairold.getValue());
									rcold.bookedby="-";
									rcold.bookingid="-";
									System.out.println("\n\n Room is booked by : "+rc.bookedby);
									roomrec.put(k,(WstRoomRecord)rcold);
				...
				else if((campus.equals("DVLSERVER")|| campus.equals("KKLSERVER") || campus.equals("WSTSERVER") && !wstbooked.equals("true")))
				{
					...
					String sentencedvl = "BOOKROOMDVL#"+newroomno+"#"+newdate+"#"+newtimeslot+"#"+username;
					sendDatadvl = sentencedvl.getBytes();
					...
					return_valuedvl=modifiedSentencedvl.substring(12,22);
					return_valuedvl=return_valuedvl.trim();
					System.out.println("Return value after dvl booking :"+return_valuedvl);
					if(!return_valuedvl.equals("Successful"))
					{
					...
					if(return_valuedvl.equals("Successful") || return_valuekkl.equals("Successful"))
					{
						...
						synchronized(this)
						{
							...
							rcold.bookedby="-";
							rcold.bookingid="-";
							System.out.println("\n\n Room is booked by : "+rcold.bookedby);
							roomrec.put(n,(WstRoomRecord)rcold);
							log="Change Successful\t"+" Room No : "+ newroomno+"\t Booking Date : "+newdate+"\t Timeslot : "+newtimeslot+"\t Booked by : "+username;
