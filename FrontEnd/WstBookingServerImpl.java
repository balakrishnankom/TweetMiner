...
class WstRoomRecord
{
	...
	public WstRoomRecord(String roomno,String date,String timeslot,String bookedby,String bookingid)
	{
		this.roomno=roomno;
		this.date=date;
...
public class WstBookingServerImpl
{
	int portno;
	
	protected static String logfile ="";
	public WstRoomRecord rr;
	HashMap<Integer,WstRoomRecord> roomrec=new HashMap<Integer,WstRoomRecord>();  
	public WstBookingServerImpl( )  {  super ( ); }
	 

	public void inithashmap(HashMap h)
	{
			this.roomrec=h;
	}
	...
	public String createRoom(String no,String date,String timeslot,String username)
	{
		...
		String isavailable="";
		logfile="E:\\WSTCampus_Server_Logs_"+username+"_log.txt";
		...
		try
		{
			...
			{	
				...
				while (it1.hasNext()) 
				{
					...
					WstRoomRecord ras=(WstRoomRecord)pair.getValue();
			...
			{
				rr=new WstRoomRecord(no,date,timeslot,bookedby,bookingid);
				synchronized(this)
				{
		...
		try
		{
			...
			{	log="";
				...
				while (it1.hasNext()) 
				{
					...
					if(ras.date.equals(date)==true && ras.timeslot.equals(timeslot)==true && ras.roomno.equals(no)==true)
					{
						synchronized(this){roomrec.remove(pair.getKey(),(WstRoomRecord)pair.getValue());}
	...
	public String getAvailableTimeSlot(String date,String username,String campus)
	{
		...
		else
		{
			try
			{
				...
				InetAddress IPAddressdvl = InetAddress.getByName("192.168.43.30");
				byte[] sendDatadvl = new byte[1024];
				byte[] receiveDatadvl = new byte[1024];
				String sentencedvl = "GETAVAILABLEROOMSDVL#"+date+"#"+username;
				...
				InetAddress IPAddresskkl = InetAddress.getByName("192.168.43.30");
				byte[] sendDatakkl = new byte[1024];
				byte[] receiveDatakkl = new byte[1024];
	    		
				String sentencekkl = "GETAVAILABLEROOMSKKL#"+date+"#"+username;
				...
				InetAddress IPAddresswst = InetAddress.getByName("192.168.43.30");
				byte[] sendDatawst = new byte[1024];
				byte[] receiveDatawst = new byte[1024];
	    
				String sentencewst = "GETAVAILABLEROOMSWST#"+date+"#"+username;
		...
		/*try (FileWriter f = new FileWriter(logfile, true); BufferedWriter b = new BufferedWriter(f); PrintWriter p = new PrintWriter(b);)*/System.out.println("\n\n\t\t Available rooms-slots in KKL campus ");
		System.out.print("\t\t------------------------------------\n\n");
		Iterator itview = roomrec.entrySet().iterator();
		while (itview.hasNext()) 
		{
	...
	public String bookRoom(String no,String date,String timeslot,String username,String campus)
	{
		...
		String modifiedSentencedvl="",modifiedSentencekkl="",modifiedSentencewst="";		
		String modifiedSentencedvlcount="",modifiedSentencewstcount="",modifiedSentencekklcount="";		
		String return_value="";
		String return_valuecount="";
		String return_valuedvl="",return_valuekkl="",return_valuewst="";
		String return_valuedvlcount="",return_valuekklcount="",return_valuewstcount="";
		...
		try
		{
				...
				{	
					...
					while (it1.hasNext()) 
					{
						...
						if(ras.date.equals(date)==true && ras.timeslot.equals(timeslot)==true && ras.roomno.equals(no)==true && ras.bookedby=="-" && booklim<3)
						{
							...
							WstRoomRecord rc=(WstRoomRecord)pair.getValue();
							synchronized(this)
							{
								roomrec.remove(pair.getKey(),(WstRoomRecord)pair.getValue());
								rc.bookedby=username;
								rc.bookingid=username+""+(int)(new Date().getTime()/1000);
								bookingid=rc.bookingid;
								roomrec.put(k,(WstRoomRecord)rc);
							}
			...
			else
			{
				try
				{
					...
					InetAddress IPAddressdvlcount = InetAddress.getByName("192.168.43.30");
					...
					InetAddress IPAddresskklcount = InetAddress.getByName("192.168.43.30");
					...
					InetAddress IPAddresswstcount = InetAddress.getByName("192.168.43.30");
		...
		try{
				...
				if(!roomrec.isEmpty())
				{
					...
					while (rese.hasNext()) 
					{
						...
						WstRoomRecord res=(WstRoomRecord)lim.getValue();
	...
	try
	{
			...
			InetAddress IPAddressdvl1 = InetAddress.getByName("192.168.43.30");
			...
			if(!return_valuedvl1.equals("Successful"))
			{
					...
					InetAddress IPAddresskkl1 = InetAddress.getByName("192.168.43.30");
			...
			if(!return_valuedvl1.equals("Successful") && !return_valuekkl1.equals("Successful"))
			{
						...
						InetAddress IPAddresswst1 = InetAddress.getByName("192.168.43.30");
			...
			if((return_valuedvl1.equals("Successful") || return_valuekkl1.equals("Successful")||return_valuewst1.equals("Successful")))
			{
				...
				if((return_valuedvl1.equals("Successful") || return_valuekkl1.equals("Successful")||return_valuewst1.equals("Successful")))
				{
							...
							InetAddress IPAddressdvl2 = InetAddress.getByName("192.168.43.30");
							...
							{					
								...
								InetAddress IPAddresskkl2 = InetAddress.getByName("192.168.43.30");
							...
							if(!return_valuecandvl2.equals("Successful") && !return_valuecankkl2.equals("Successful"))
							{
								...
								InetAddress IPAddresswst2 = InetAddress.getByName("192.168.43.30");
				...
				if((!return_valuecandvl2.equals("Successful") && !return_valuecankkl2.equals("Successful")&&!return_valuecanwst2.equals("Successful")))
				{
							...
							InetAddress IPAddressdvl3 = InetAddress.getByName("192.168.43.30");
							...
							{					
								...
								InetAddress IPAddresskkl3 = InetAddress.getByName("192.168.43.30");
							...
							if(!return_valuecandvl3.equals("Successful") && !return_valuecankkl3.equals("Successful"))
							{
								...
								InetAddress IPAddresswst3 = InetAddress.getByName("192.168.43.30");
...
public static void main(String args[]) 
{
	...
	final String INET_ADDR = "224.4.5.6";
	final int PORT =9001;
	...
	{	 
			public void run()
			{
				try
				{
					...
					System.out.println("WST server started:");
					...
					try (MulticastSocket clientSocket = new MulticastSocket(PORT))
					{
						...
						while (true) 
						{
							...
							String bookid;
							String roomno,date,timeslot,username,campus;
							if(data[1].equals("CL"))
							{
								switch(Integer.parseInt(data[2]))
								{
										case 1:
												...
												timeslot=data[5];
												username=data[6];
												result=addobj.bookRoom(roomno,date,timeslot,username,username.substring(0,3));
												break;
										case 2:
												date=data[4];
												username=data[5];
												result=addobj.getAvailableTimeSlot(date,username,username.substring(0,3));
												break;
										case 3:
										
												bookid=data[3];
												username=data[4];
												result=addobj.cancelBooking(bookid,username,username.substring(0,3));
												break;
										case 4:
												bookid=data[3];
												username=data[4];
												...
												date=data[7];
												timeslot=data[8];
												result=addobj.changeReservation(bookid,username,campus,roomno,date,timeslot);	
												break;
				...
				InetAddress compareResult = InetAddress.getByName("192.168.43.30"); 
				InetAddress wstsuy = InetAddress.getByName("192.168.43.31"); 
	
					DatagramPacket reply=new DatagramPacket(result.getBytes(),result.getBytes().length,compareResult,8003);
						}
					...
					while(true)
					{
						...
						if(s.equals("TH"))
						{
							/*try {
									...
									buffer = "RH".getBytes();
									DatagramPacket request = new DatagramPacket(buffer1, buffer1.length,wstsuy,9979);
									sock.send(request);
									byte[] buffer2 = new byte[1024];
									DatagramPacket reply = new DatagramPacket(buffer2, buffer2.length);
									sock.receive(reply);
									buffer2 = reply.getData();
									ByteArrayInputStream bis = new ByteArrayInputStream(buffer2);
									ObjectInputStream oin = new ObjectInputStream(bis);
									HashMap<Integer,WstRoomRecord> roomrec=new HashMap<Integer,WstRoomRecord>();  
	

									HashMap<Date, HashMap<String, ArrayList<Slots>>> temp = (HashMap) oin.readObject();
									addobj.initHashMap(temp);
								} catch (Exception e) {
									e.printStackTrace();
								}	
							*/
						...
						if(s.equals("RH"))
						{
							/*
								...
								HashMap<Date, HashMap<String, ArrayList<Slots>>> temp = addobj.returnHashmap();
								ByteArrayOutputStream bout = new ByteArrayOutputStream();
								ObjectOutputStream out = new ObjectOutputStream(bout);
								out.writeObject(temp);
								buffer = bout.toByteArray();
								DatagramPacket reply = new DatagramPacket(buffer3, buffer3.length, request.getAddress(),
								request.getPort());
								sock.send(reply);*/
						}
						...
						if(tempcancel.equals("CANCELROOMWST"))
						{
							...
							String cancelresult="";
							cancelresult=addobj.cancelBooking(tmpcanbookingid,tmpcanusername,"WSTSERVER");
