...
class WstRoomRecord
{
	String roomno,date,timeslot,bookedby;
	int bookingid;
	public WstRoomRecord(String roomno,String date,String timeslot,String bookedby,int bookingid)
	{
		this.roomno=roomno;
		this.date=date;
...
System.out.println(isCreated);		return isCreated;
	...
	public int bookRoom(String no,String date,String timeslot,String username,String campus,int fla)
	{
		...
		String check="false";
		logfile="E:\\WSTCampus_Server_Logs_"+username+"_log.txt";
		int count= (int) (new Date().getTime()/10000);
		String log="",flag="",bookedby="-";
		int bookingid=0;
		int max=0;
		int booklim=0;
		int bid=0;
					...
					while (it1.hasNext()) 
					{
						...
						if(ras.date.equals(date)==true && ras.timeslot.equals(timeslot)==true && ras.roomno.equals(no)==true && ras.bookedby=="-" && booklim<3)
						{
							...
							synchronized(this)
							{
								...
								rc.bookingid=(int)(new Date().getTime()/10000);
			...
			else
			{
				try
				{
					...
					if((countdvl+countkkl+countwst)<3)
					{
							...
							return_valuedvl=modifiedSentencedvl;
							return_valuedvl=return_valuedvl.trim();
							if(!return_valuedvl.equals("0"))
							{
								...
								return_valuekkl=modifiedSentencekkl;
								return_valuekkl=return_valuekkl.trim();
						
							}		
							if(!return_valuedvl.equals("0") && !return_valuekkl.equals("0"))
							{
								//WST Available time slots
								DatagramSocket clientSocketwst = new DatagramSocket();
								InetAddress IPAddresswst = InetAddress.getByName("localhost");
								...
								return_valuewst=modifiedSentencewst;
								return_valuewst=return_valuewst.trim();
							}
							
							if(!return_valuedvl.equals("0"))
								log=modifiedSentencedvl;
							else if(!return_valuekkl.equals("0"))
								log=modifiedSentencekkl;
							else if(!return_valuewst.equals("0"))
	...
	public boolean cancelBooking (int booking_id,String username,String campus)
	{
			...
			try
			{
				...
				{	
					...
					while (it1.hasNext()) 
					{
						Map.Entry pair = (Map.Entry)it1.next();
						WstRoomRecord ras=(WstRoomRecord)pair.getValue();
						if((ras.bookedby.equals(username)) && (ras.bookingid==booking_id))
						{
							int k=(int)pair.getKey();
							WstRoomRecord rc=(WstRoomRecord)pair.getValue();
							synchronized(this){
		...
		else
		{
			try
			{
				...
				if(!return_valuecandvl.equals("true"))
				{					
					//KKL Cancel booking
					DatagramSocket clientSocketkkl = new DatagramSocket();
					InetAddress IPAddresskkl = InetAddress.getByName("localhost");
				...
				if(!return_valuecandvl.equals("true") && !return_valuecankkl.equals("true"))
				{
					//WST cancel booking
					DatagramSocket clientSocketwst = new DatagramSocket();
					InetAddress IPAddresswst = InetAddress.getByName("localhost");
				...
				if(return_valuecandvl.equals("true"))
					log=modifiedSentencedvl;
				else if(return_valuecankkl.equals("true"))
					log=modifiedSentencekkl;
				else if(return_valuecanwst.equals("true"))
					log=modifiedSentencewst;
			}catch(Exception e){}
 ...
 public long changeReservation(String booking_id,String username,String campus,String newroomno,String newdate,String newtimeslot)
 {
